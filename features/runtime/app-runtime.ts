...
import type { AppPreferencesRecord, DataDeletionPort } from '@/features/domain/contracts';
import { domainError, normalizeError } from '@/features/domain/errors';
import { createReportHandler, createTerminalFailureHandler, createTranscriptionHandler, type ProcessingHandlerDependencies } from '@/features/jobs/handlers';
...
import { readPreferences, providerCredentials } from '@/features/onboarding/storage';
...
function requireAndroid() {
  if (Platform.OS !== 'android') {
    throw domainError('unsupported', 'database', 'Hmmmidea is available on Android only.');
  }
}

async function setupSnapshot(database: AppDatabase): Promise<AppRuntimeSnapshot> {
...
function runtimeProcessingDependencies(database: AppDatabase): ProcessingHandlerDependencies {
  return {
...
async function hydrate(): Promise<AppRuntime> {
  ...
  await cancelPendingGenericNotifications();
...
export function subscribeAppRuntime(listener: (next: AppRuntimeSnapshot) => void) {
...
export async function retryCaptureProcessing(captureId: string) {
  const appRuntime = await bootAppRuntime();
  const generation = await appRuntime.database.repositories.deletions.getGeneration();
  const capture = await appRuntime.database.repositories.captures.get(captureId);
  ...
  const jobs = await appRuntime.database.repositories.jobs.listForCapture(captureId);
  const failed = [...jobs]
  ...
  const retryJob = !failed || failed.kind === 'transcribe-capture'
    ? capture.audio
      ? transcriptionJob({
        captureId,
        generation,
        revision,
        requestId: failed ? `${failed.requestId}:retry` : `${captureId}:recovery:transcription`,
        audio: capture.audio,
      ...
      : null
    : failed.kind === 'generate-report' && capture.transcript?.phase === 'final'
      ? reportJob({
        captureId,
        generation,
        revision,
  ...
  await appRuntime.database.repositories.captures.setProcessingState(captureId, 'queued', null, now, generation);
  appRuntime.wake();
  return true;
}
