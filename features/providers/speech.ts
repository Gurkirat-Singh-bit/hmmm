...
function fileSpeechProvider(providerId: 'openai' | 'groq' | 'custom'): SpeechProviderPort {
  return {
    descriptor: { ...SPEECH_PROVIDER_DESCRIPTORS[providerId], kind: 'speech' },
    transcribe: (context, request) => transcribeOpenAiCompatible(providerId, context, request),
  };
}

async function transcribeDeepgram(context: ProviderContext, request: TranscriptionRequest) {
...
async function transcribeOpenAiCompatible(
  ...
  if (request.languageTag) form.append('language', primaryLanguage(request.languageTag));
  const payload = await requestJson({
    providerId,
    operation: 'transcription',
...
async function transcribeOpenRouter(context: ProviderContext, request: TranscriptionRequest) {
  ...
  const payload = await requestJson({
    ...
    init: {
      ...
      body: JSON.stringify({
        ...
        ...(request.languageTag ? { language: primaryLanguage(request.languageTag) } : {}),
        temperature: 0,
      }),
    },
    timeoutMs: PROVIDER_TIMEOUT_MS.speech,
...
async function transcribeGemini(context: ProviderContext, request: TranscriptionRequest) {
      ...
      body: JSON.stringify({
        contents: [{
          ...
          parts: [
            ...
            { inlineData: { mimeType: request.audio.mimeType, data: await new File(request.audio.uri).base64() } },
          ],
        }],
  ...
  return {
    text: boundedTranscript(requireResponseText(extractGeminiText(payload), 'google', 'transcription'), 'google'),
...
async function openDeepgramSession(
  context: ProviderContext,
  request: Readonly<{ requestId: string; mimeType: string; languageTag: string | null }>,
) {
  const { apiKey, model } = requireProviderContext(context, 'deepgram');
  ...
  if (request.languageTag) query.set('language', request.languageTag);
  const socket = new WebSocket(`${PROVIDER_ENDPOINTS.deepgramLive}?${query}`, ['token', apiKey]);
...
class DeepgramLiveSession implements LiveTranscriptionSessionPort {
  ...
  constructor(private readonly socket: WebSocket, private readonly languageTag: string | null) {
    socket.onmessage = (event) => this.onMessage(event.data);
    socket.onerror = () => this.fail('The live transcription connection failed.');
  ...
  async sendAudio(chunk: Uint8Array, sequence: number) {
    if (this.closed || this.socket.readyState !== WebSocket.OPEN) {
      throw providerError('provider-unavailable', 'transcription', 'deepgram', 'The live transcription connection is closed.', true);
    }
  ...
  finish() {
    ...
    if (this.closed || this.socket.readyState !== WebSocket.OPEN) {
      return Promise.reject(providerError('provider-unavailable', 'transcription', 'deepgram', 'The live transcription connection is closed.', true));
    }
  ...
  async cancel() {
    ...
    this.socket.close();
    this.rejectFinish?.(providerError('cancelled', 'transcription', 'deepgram', 'Live transcription was cancelled.'));
    this.clearFinish();
  ...
  private onMessage(data: unknown) {
    ...
    if (data.length > PROVIDER_RESPONSE_LIMITS.streamEventCharacters) {
      this.failWith(providerError('invalid-provider-output', 'transcription', 'deepgram', 'Deepgram returned an oversized live response.', true));
      return;
    }
    let payload: unknown;
    try {
    ...
    if (!isRecord(payload) || payload.type !== 'Results' || !isRecord(payload.channel) || !Array.isArray(payload.channel.alternatives)) return;
    const alternative = payload.channel.alternatives[0];
    if (!isRecord(alternative) || typeof alternative.transcript !== 'string') return;
    const text = alternative.transcript.trim();
    if (text.length > PROVIDER_RESPONSE_LIMITS.transcriptCharacters) {
      this.failWith(providerError('invalid-provider-output', 'transcription', 'deepgram', 'Deepgram returned an oversized transcript.', true));
      return;
    }
    const final = payload.is_final === true;
    if (final && text) {
      if (this.finalParts.length >= PROVIDER_RESPONSE_LIMITS.transcriptSegments
        || this.transcriptCharacters + text.length > PROVIDER_RESPONSE_LIMITS.transcriptCharacters) {
        this.failWith(providerError('invalid-provider-output', 'transcription', 'deepgram', 'Deepgram returned an oversized transcript.', true));
        return;
      }
      ...
      try {
        ...
        this.failWith(error instanceof Error ? error : providerError('invalid-provider-output', 'transcription', 'deepgram', 'Deepgram returned invalid transcript segments.', true));
        return;
      }
      if (this.segments.length + words.length > PROVIDER_RESPONSE_LIMITS.transcriptSegments) {
        this.failWith(providerError('invalid-provider-output', 'transcription', 'deepgram', 'Deepgram returned too many transcript segments.', true));
        return;
      }
    ...
    if (text) this.emit({ type: 'transcript', phase: final ? 'final' : 'provisional', text, sequence: ++this.eventSequence });
    if (payload.from_finalize === true && this.socket.readyState === WebSocket.OPEN) {
  ...
  private onClose(event: { code?: number }) {
    ...
    if (this.finishing && event.code !== 1_000) {
      this.rejectFinish?.(providerError('provider-unavailable', 'transcription', 'deepgram', 'The live transcription connection closed early.', true));
      this.clearFinish();
      this.emit({ type: 'closed' });
      return;
    }
    const result = { text: this.finalParts.join(' ').trim(), languageTag: this.languageTag, segments: this.segments };
  ...
  private fail(message: string) {
    this.failWith(providerError('provider-unavailable', 'transcription', 'deepgram', message, true));
  }
...
function parseDeepgramTranscript(payload: unknown, fallbackLanguage: string | null): FinalTranscript {
  if (!isRecord(payload) || !isRecord(payload.results) || !Array.isArray(payload.results.channels)) {
    throw providerError('invalid-provider-output', 'transcription', 'deepgram', 'Deepgram returned an invalid transcript.', true);
  }
  const channel = payload.results.channels[0];
  const alternative = isRecord(channel) && Array.isArray(channel.alternatives) ? channel.alternatives[0] : null;
  ...
  return {
    text,
    languageTag: isRecord(channel) && typeof channel.detected_language === 'string'
      ? boundedLanguage(channel.detected_language, 'deepgram')
      : fallbackLanguage,
    segments: utterances.length ? utterances : parseWords(alternative.words, 'deepgram'),
  };
}

function parseOpenAiTranscript(payload: unknown, providerId: string, fallbackLanguage: string | null): FinalTranscript {
  if (!isRecord(payload) || typeof payload.text !== 'string') {
    throw providerError('invalid-provider-output', 'transcription', providerId, 'The provider returned an invalid transcript.', true);
  }
  return {
    text: boundedTranscript(payload.text, providerId),
    languageTag: typeof payload.language === 'string' ? boundedLanguage(payload.language, providerId) : fallbackLanguage,
    segments: Array.isArray(payload.segments) ? parseTimedSegments(payload.segments, providerId) : [],
  };
}

function parseTimedSegments(value: unknown[], providerId: string): TranscriptSegment[] {
  if (value.length > PROVIDER_RESPONSE_LIMITS.transcriptSegments) {
    throw providerError('invalid-provider-output', 'transcription', providerId, 'The provider returned too many transcript segments.', true);
  }
  return value.flatMap((item) => parseTimedSegment(item, providerId));
}

function parseTimedSegment(value: unknown, providerId: string): TranscriptSegment[] {
  if (!isRecord(value) || typeof value.start !== 'number' || typeof value.end !== 'number' || typeof value.text !== 'string') return [];
  if (value.text.length > PROVIDER_RESPONSE_LIMITS.structuredStringCharacters) {
    throw providerError('invalid-provider-output', 'transcription', providerId, 'The provider returned an oversized transcript segment.', true);
  }
  const confidence = typeof value.confidence === 'number' ? value.confidence : null;
  return [{ startMs: Math.max(0, Math.round(value.start * 1_000)), endMs: Math.max(0, Math.round(value.end * 1_000)), text: value.text.trim(), confidence }];
}
...
function parseWords(value: unknown, providerId: string) {
  ...
  return value.flatMap((word) => {
    if (!isRecord(word) || typeof word.start !== 'number' || typeof word.end !== 'number') return [];
    const text = typeof word.punctuated_word === 'string'
      ? word.punctuated_word
      : typeof word.word === 'string' ? word.word : null;
    if (!text) return [];
...
function boundedTranscript(value: string, providerId: string) {
  ...
  if (text.length > PROVIDER_RESPONSE_LIMITS.transcriptCharacters) {
    throw providerError('invalid-provider-output', 'transcription', providerId, 'The provider returned an oversized transcript.', true);
  }
  return text;
}
...
function boundedLanguage(value: string, providerId: string) {
  if (value.length > PROVIDER_RESPONSE_LIMITS.structuredStringCharacters) {
    throw providerError('invalid-provider-output', 'transcription', providerId, 'The provider returned an invalid language tag.', true);
  }
  return value;
}
...
function audioFormat(container: string, mimeType: string) {
  ...
  if (['wav', 'mp3', 'flac', 'm4a', 'ogg', 'webm', 'aac'].includes(normalized)) return normalized;
...
function waitForSocket(socket: WebSocket) {
  return new Promise<void>((resolve, reject) => {
    const timeout = setTimeout(() => {
      socket.close();
      reject(providerError('timeout', 'transcription', 'deepgram', 'Deepgram took too long to open a live session.', true));
    ...
    socket.onerror = () => {
      clearTimeout(timeout);
      reject(providerError('provider-unavailable', 'transcription', 'deepgram', 'Deepgram could not open a live session.', true));
    };
  });
}
...
async function waitForSocketCapacity(socket: WebSocket, incomingBytes: number) {
  ...
  while (
    ...
    && socket.bufferedAmount + incomingBytes > DEEPGRAM_LIVE_AUDIO.maxBufferedAmount
  ) {
    ...
    await new Promise<void>((resolve) => setTimeout(resolve, DEEPGRAM_LIVE_AUDIO.bufferPollMs));
  }
}
