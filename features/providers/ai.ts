...
function aiProvider(providerId: AiProviderId): AiProviderPort {
  const descriptor = { ...AI_PROVIDER_DESCRIPTORS[providerId], kind: 'ai' as const };
  return {
    descriptor,
    ...(descriptor.capabilities['ai.research-with-citations']
      ? { research: (context: ProviderContext, request: ResearchRequest) => research(providerId, context, request) }
      : {}),
    generateReport: (context, request) => generateReport(providerId, context, request),
    completeDiscussion: (context, request) => completeDiscussion(providerId, context, request),
    streamDiscussion: (context, request) => streamDiscussion(providerId, context, request),
  };
}
...
async function research(
  ...
  const { apiKey, model } = requireProviderContext(context, providerId);
  if (!supportsProviderResearch(providerId, model)) return { findings: [], sources: [] };
  const payload = await requestJson({
    providerId,
    operation: 'research',
    url: generationUrl(providerId, context, model, false),
...
function reportBody(providerId: AiProviderId, model: string, request: ReportGenerationRequest) {
  ...
  if (providerId === 'openai') {
    return {
      ...
      instructions: system,
      input: user,
      text: { format: { type: 'json_schema', name: 'idea_report', strict: true, schema: REPORT_JSON_SCHEMA } },
    };
  }
  ...
  if (providerId === 'claude') {
    return { model, max_tokens: 3_000, system, messages: [{ role: 'user', content: user }] };
  }
  return {
    model,
...
function discussionBody(providerId: AiProviderId, model: string, request: DiscussionRequest) {
  ...
  if (providerId === 'openai') {
    return {
      ...
      instructions: system,
      input: messages,
      text: { format: { type: 'json_schema', name: 'discussion_reply', strict: true, schema: DISCUSSION_JSON_SCHEMA } },
      stream: true,
    };
  }
  if (providerId === 'google') {
...
function generationUrl(providerId: AiProviderId, context: ProviderContext, model: string, stream: boolean) {
...
function streamTextDelta(providerId: AiProviderId, payload: Record<string, unknown>) {
  if (providerId === 'openai') {
    return payload.type === 'response.output_text.delta' && typeof payload.delta === 'string' ? payload.delta : null;
  }
  ...
  if (!Array.isArray(payload.choices) || !isRecord(payload.choices[0]) || !isRecord(payload.choices[0].delta)) return null;
  return typeof payload.choices[0].delta.content === 'string' ? payload.choices[0].delta.content : null;
}
  ...
  for (const item of output) {
    ...
    if (Array.isArray(item.content)) {
      for (const part of boundedResearchArray(item.content, 'openai')) {
        ...
        if (part.text !== undefined && typeof part.text !== 'string') throw invalidResearchOutput('openai');
        if (typeof part.text === 'string' && part.text.length > PROVIDER_RESPONSE_LIMITS.findingCharacters) {
          throw invalidResearchOutput('openai');
        }
        if (!annotations.length) continue;
        if (typeof part.text !== 'string') throw invalidResearchOutput('openai');
        const text = part.text;
        for (const annotation of annotations) citations.push(...citationFromAnnotation(annotation, text, 'openai'));
      }
    } else if (item.content !== undefined) {
      throw invalidResearchOutput('openai');
    }
    if (item.type === 'web_search_call') {
      if (item.action !== undefined && !isRecord(item.action)) throw invalidResearchOutput('openai');
...
function parseOpenRouterResearch(payload: unknown) {
  ...
  if (!isRecord(choice) || !isRecord(choice.message)) throw invalidResearchOutput('openrouter');
  const content = messageContent(choice.message);
  if (content === null || (content && content.length > PROVIDER_RESPONSE_LIMITS.findingCharacters)) {
  ...
  return normalizeCitations(annotations.flatMap((annotation) => citationFromAnnotation(annotation, text, 'openrouter')), 'openrouter');
}
...
function parseAnthropicResearch(payload: unknown) {
  ...
  for (const block of content) {
    if (!isRecord(block) || typeof block.type !== 'string') throw invalidResearchOutput('claude');
    if (block.type !== 'text') {
      if (block.citations !== undefined) throw invalidResearchOutput('claude');
      continue;
    }
    if (typeof block.text !== 'string' || block.text.length > PROVIDER_RESPONSE_LIMITS.findingCharacters) throw invalidResearchOutput('claude');
    ...
    for (const citation of boundedResearchArray(block.citations, 'claude')) {
      ...
      citations.push({ url: citation.url, title: citation.title, text: citation.cited_text });
    }
  }
...
function parseGroqResearch(payload: unknown) {
  ...
  if (!choices.length) return { findings: [], sources: [] };
  const choice = choices[0];
  if (!isRecord(choice) || !isRecord(choice.message)) throw invalidResearchOutput('groq');
  ...
  if (typeof choice.message.content === 'string' && choice.message.content.length > PROVIDER_RESPONSE_LIMITS.findingCharacters) {
    throw invalidResearchOutput('groq');
  }
  ...
  for (const tool of boundedTools) {
    ...
    if (tool.search_results !== undefined && !isRecord(tool.search_results)) throw invalidResearchOutput('groq');
    if (isRecord(tool.search_results)) {
      const results = tool.search_results.results === undefined
        ? []
        : boundedResearchArray(tool.search_results.results, 'groq');
      for (const result of results) {
        if (!isRecord(result)) throw invalidResearchOutput('groq');
        citations.push({ url: result.url, title: result.title, text: result.content });
      }
    }
    if (tool.output !== undefined && tool.output !== null && typeof tool.output !== 'string') throw invalidResearchOutput('groq');
...
function parseGeminiResearch(payload: unknown) {
  ...
  for (const part of boundedResearchArray(candidate.content.parts, 'google')) {
    if (!isRecord(part) || (part.text !== undefined && typeof part.text !== 'string')) throw invalidResearchOutput('google');
    if (typeof part.text === 'string' && part.text.length > PROVIDER_RESPONSE_LIMITS.findingCharacters) throw invalidResearchOutput('google');
  }
  const metadata = candidate.groundingMetadata;
  if (metadata === undefined) return { findings: [], sources: [] };
  if (!isRecord(metadata) || !Array.isArray(metadata.groundingChunks) || !Array.isArray(metadata.groundingSupports)) {
    throw invalidResearchOutput('google');
  }
  const groundingChunks = boundedResearchArray(metadata.groundingChunks, 'google');
  const groundingSupports = boundedResearchArray(metadata.groundingSupports, 'google');
  const citations: CitationInput[] = [];
  for (const support of groundingSupports) {
    if (!isRecord(support) || !isRecord(support.segment) || typeof support.segment.text !== 'string'
      || support.segment.text.length > PROVIDER_RESPONSE_LIMITS.findingCharacters
      || !Array.isArray(support.groundingChunkIndices)) {
      throw invalidResearchOutput('google');
    }
    for (const index of boundedResearchArray(support.groundingChunkIndices, 'google')) {
      if (typeof index !== 'number' || !Number.isInteger(index) || index < 0 || index >= groundingChunks.length) {
        throw invalidResearchOutput('google');
      }
      const chunk = groundingChunks[index];
      if (!isRecord(chunk) || !isRecord(chunk.web)) throw invalidResearchOutput('google');
      citations.push({ url: chunk.web.uri, title: chunk.web.title, text: support.segment.text });
    }
  }
  return normalizeCitations(citations, 'google');
}

function citationFromAnnotation(annotation: unknown, text: string, providerId: string): CitationInput[] {
  if (!isRecord(annotation)) throw invalidResearchOutput(providerId);
  if (annotation.url_citation !== undefined && !isRecord(annotation.url_citation)) throw invalidResearchOutput(providerId);
  const citation = isRecord(annotation.url_citation) ? annotation.url_citation : annotation;
  if (citation.url === undefined || typeof citation.url !== 'string') throw invalidResearchOutput(providerId);
  const start = typeof citation.start_index === 'number' && Number.isInteger(citation.start_index) ? citation.start_index : null;
  const end = typeof citation.end_index === 'number' && Number.isInteger(citation.end_index) ? citation.end_index : null;
  if ((citation.start_index !== undefined && start === null) || (citation.end_index !== undefined && end === null)) {
    throw invalidResearchOutput(providerId);
  }
  const citedText = typeof citation.cited_text === 'string'
    ? citation.cited_text
    : start !== null && end !== null && start >= 0 && end > start && end <= text.length
      ? text.slice(start, end)
      : null;
  if (citation.cited_text !== undefined && citation.cited_text !== null && typeof citation.cited_text !== 'string') {
    throw invalidResearchOutput(providerId);
  }
...
function citationsFromToolOutput(value: unknown, providerId: string): CitationInput[] {
  if (typeof value !== 'string') return [];
  const urls = value.match(/https:\/\/[^\s<>"']+/g) ?? [];
  if (urls.length > PROVIDER_RESPONSE_LIMITS.researchCitations) throw invalidResearchOutput(providerId);
  return [...new Set(urls.map((url) => url.replace(/[),.;:!?\]]+$/, '')))]
    .map((url) => ({ url, title: null, text: null }));
}
...
function parseStreamPayload(data: string, providerId: string) {
  ...
  throw providerError('invalid-provider-output', 'discussion', providerId, 'The provider returned an invalid response stream.', true);
}

function researchArray(payload: unknown, property: string, providerId: string): unknown[] {
  if (!isRecord(payload) || !Array.isArray(payload[property])) throw invalidResearchOutput(providerId);
...
function boundedResearchArray(value: unknown, providerId: string): unknown[] {
  if (!Array.isArray(value) || value.length > PROVIDER_RESPONSE_LIMITS.researchCitations) {
    throw invalidResearchOutput(providerId);
  }
  return value;
}
...
function assertNoStreamError(payload: Record<string, unknown>, providerId: string) {
  if (payload.type !== 'error' && payload.type !== 'response.failed' && !payload.error) return;
  throw providerError('provider-unavailable', 'discussion', providerId, 'The provider interrupted the response stream.', true);
}
