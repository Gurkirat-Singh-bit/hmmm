import type { ProviderCapabilities, ProviderDescriptor, ProviderKind } from '../domain/providers';
...
const RESEARCH_MODEL_PATTERNS: Readonly<Record<Exclude<AiProviderId, 'custom'>, readonly RegExp[]>> = {
  claude: [
...
export function supportsProviderResearch(providerId: string, model: string) {
  if (providerId === 'custom' || !(providerId in RESEARCH_MODEL_PATTERNS)) return false;
  return RESEARCH_MODEL_PATTERNS[providerId as keyof typeof RESEARCH_MODEL_PATTERNS]
