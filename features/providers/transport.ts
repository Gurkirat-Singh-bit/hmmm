...
export function requireProviderContext(context: ProviderContext, providerId: ProviderId) {
...
export function providerBaseUrl(context: ProviderContext, providerId: ProviderId) {
  if (providerId === 'custom') return normalizeCustomEndpoint(context.selection.endpoint);
  if (providerId === 'claude') return PROVIDER_ENDPOINTS.anthropic;
  return PROVIDER_ENDPOINTS[providerId];
}

export function providerHeaders(providerId: ProviderId, apiKey: string, json = true): Record<string, string> {
...
export async function requestJson(options: RequestOptions): Promise<unknown> {
  ...
  try {
    return await readJsonResponse(response, options.providerId, options.operation);
  } catch (reason) {
...
export async function readJsonResponse(response: Response, providerId: string, operation: AppOperation) {
  try {
...
export async function requestProbe(
  ...
  try {
    if (!response.response.ok && !acceptedStatuses.includes(response.response.status)) {
      throw statusError(options, response.response.status);
    }
...
export async function* streamSse(options: RequestOptions): AsyncGenerator<SseMessage> {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    try {
      ...
      if (!call.response.ok) {
        const error = statusError(options, call.response.status);
...
async function requestResponse(options: RequestOptions) {
  ...
  throw providerError('unknown', options.operation, options.providerId, 'The provider request failed.', true);
}
...
async function fetchWithTimeout(options: RequestOptions) {
  ...
  try {
    const response = await fetch(options.url, { redirect: 'error', ...options.init, body: options.init.body ?? undefined, signal: controller.signal });
    return {
      response,
      dispose: () => {
...
async function readBoundedResponseText(response: Response, providerId: string, operation: AppOperation) {
  const declaredLength = Number(response.headers.get('content-length'));
  if (Number.isFinite(declaredLength) && declaredLength > PROVIDER_RESPONSE_LIMITS.responseBytes) {
    throw providerError(
      'invalid-provider-output',
      operation,
      providerId,
  ...
  if (response.body) {
    ...
    try {
      while (true) {
        ...
        if (value?.byteLength) chunks.push(decoder.decode(value, { stream: !done }));
        if (done) {
  ...
  if (typeof response.text === 'function') {
    ...
    if (new TextEncoder().encode(text).byteLength > PROVIDER_RESPONSE_LIMITS.responseBytes) {
      throw providerError(
        'invalid-provider-output',
        operation,
        providerId,
  ...
  if (typeof response.json === 'function') {
    ...
    if (typeof text !== 'string' || new TextEncoder().encode(text).byteLength > PROVIDER_RESPONSE_LIMITS.responseBytes) {
      throw providerError(
        'invalid-provider-output',
        operation,
        providerId,
...
function statusError(options: RequestOptions, status: number) {
  ...
  let message = 'The provider rejected the request. Check the endpoint and model.';
  let retryable = false;
  ...
  return providerError(code, options.operation, options.providerId, message, retryable, status);
}
...
function retryDelay(response: Response, attempt: number) {
  ...
  return Math.min(PROVIDER_RETRY.baseDelayMs * (attempt + 1), PROVIDER_RETRY.maxDelayMs);
}
