...
import type { LiveTranscriptEvent, ProviderRegistryPort } from '../domain/providers';
import type { CaptureHomeState, CapturePresentation } from './contracts';
...
const initialState: CaptureHomeState = { capture: initialPresentation, recent: [] };
...
type DiscoveredRecording = RecordingRecoveryCandidate & Readonly<{ location: 'draft' | 'capture' }>;
...
class CaptureController {
  ...
  async start() {
    await this.runExclusive(async () => {
      ...
      this.publish({ capture: { ...initialPresentation, phase: 'permission', message: 'Checking microphone access…' } });
      const permission = await this.recorder.getPermission();
      const granted = permission === 'granted' ? permission : await this.recorder.requestPermission();
      if (granted !== 'granted') {
        throw domainError('permission-denied', 'recording', 'Microphone access is required to record. Allow it in Android settings, then try again.', true);
      }
      ...
      this.publish({ capture: { ...initialPresentation, phase: 'starting', transcriptMode: liveMode(configured.preferences, this.providers) } });
      let session: RecordingSessionPort;
      try {
        session = await this.recorder.start({ draftId: draft.id, captureId, recoveryId: draft.recoveryId });
      } catch (error) {
      ...
      active.unsubscribe = session.subscribe((event) => {
        if (event.type === 'duration') {
          this.publish({ capture: { ...this.state.capture, elapsedMs: event.durationMs } });
          return;
        }
        if (event.type === 'state') {
          if (event.state === 'paused') this.persistLater(active, { status: 'paused' });
          if (event.state === 'recording') {
            this.persistLater(active, { status: 'recording' });
            this.publish({ capture: { ...this.state.capture, phase: 'recording', message: null } });
          }
          return;
        }
        if (event.type === 'interrupted') void this.recordingInterrupted(active, event.error);
      });
      this.active = active;
      this.publish({ capture: { ...initialPresentation, phase: 'recording', transcriptMode: liveMode(configured.preferences, this.providers) } });
  ...
  async pause() {
    await this.runExclusive(async () => {
      ...
      this.publish({ capture: { ...this.state.capture, phase: 'paused', message: null } });
    });
  }
  ...
  async cancel() {
    await this.runExclusive(async () => {
      ...
      await this.repositories.recordingDrafts.delete(active.draftId, active.generation);
      active.unsubscribe();
      this.active = null;
  ...
  async finish() {
    await this.runExclusive(async () => {
      const active = this.requireActive();
      this.publish({ capture: { ...this.state.capture, phase: 'saving', message: null } });
      try {
        const audio = await active.session.finish();
        const liveFinal = await active.live?.finish();
        const transcript = finalLiveTranscript(active, liveFinal?.text ?? '', liveFinal?.languageTag ?? null, liveFinal?.segments ?? []);
        ...
        await this.updateDraft(active, { audio: retainedAudio, durationMs: retainedAudio.durationMs });
        ...
        this.publish({
          capture: {
            ...initialPresentation,
            phase: 'queued',
            elapsedMs: retainedAudio.durationMs,
            transcript: (transcript ?? active.draft.transcript)?.text ?? '',
            transcriptMode: liveMode(active.preferences, this.providers),
          },
        });
  ...
  async retry() {
    if (this.active) {
      if (this.active.session.getState() === 'failed' || this.active.session.getState() === 'stopped') {
  ...
  private async startup() {
    ...
    this.unsubscribeDatabase = appRuntime.database.subscriptions.subscribe((change) => {
      if (change.table === 'captures' || change.table === 'jobs' || change.table === 'reports') void this.refreshRecent();
    });
    await this.reconcileInterruptedDrafts();
    await this.refreshRecent();
  }
  ...
  private async syncProfile(required: boolean) {
    ...
    if (!speechCredential?.secret.trim() || !preferences.speechProvider.model.trim() || !aiCredential?.secret.trim() || !preferences.aiProvider.model.trim()) {
      if (required) throw domainError('configuration-missing', 'provider-configuration', 'Finish configuring speech and AI providers before recording.', true);
      return { preferences };
    }
    if (!safeEndpoint(preferences.speechProvider.endpoint) || !safeEndpoint(preferences.aiProvider.endpoint)) {
      if (required) throw domainError('configuration-missing', 'provider-configuration', 'Provider endpoints must be valid HTTPS addresses.', true);
      return { preferences };
    }
    const speech = this.providers.getSpeech(preferences.speechProvider.providerId);
    const ai = this.providers.getAi(preferences.aiProvider.providerId);
    if (!speech?.descriptor.capabilities['speech.file-transcription'] || !ai?.descriptor.capabilities['ai.report-generation']) {
      if (required) throw domainError('configuration-missing', 'provider-configuration', 'Choose supported speech and AI providers before recording.', true);
      return { preferences };
    }
    return { preferences };
  }
  ...
  private openLiveTranscript(active: ActiveRecording) {
    const provider = this.providers.getSpeech(active.preferences.speechProvider.providerId);
    if (!provider?.descriptor.capabilities['speech.streaming-transcription'] || !provider.openLiveSession) return;
    const live = new RecordingLiveTranscription(active.session, async () => provider.openLiveSession!({
      selection: active.preferences.speechProvider,
      apiKey: (await this.secrets.readActive('speech'))?.secret.trim() || null,
    }, {
  ...
  private liveEvent(active: ActiveRecording, event: LiveTranscriptEvent) {
    ...
    if (event.type === 'closed') {
      this.publish({ capture: { ...this.state.capture, message: 'Live text paused. Your saved audio will be transcribed after you finish.' } });
      return;
    }
    const text = event.phase === 'final'
    ...
    this.publish({ capture: { ...this.state.capture, transcript: text, transcriptMode: 'live', message: null } });
  }

  private async recordingInterrupted(active: ActiveRecording, error: NormalizedError) {
    if (this.active !== active) return;
    try {
  ...
  private async reconcileInterruptedDrafts() {
    ...
    for (const draft of drafts) {
      ...
      if (existing) {
        ...
        await this.repositories.recordingDrafts.delete(draft.id, generation).catch(() => undefined);
        continue;
      }

      const candidate = candidateForDraft(draft, candidates);
      if ((!candidate || !candidate.audio) && draft.status === 'failed' && draft.error) continue;
      const now = new Date().toISOString();
      ...
      try {
        ...
        await this.repositories.captures.commitRecording({
          draftId: draft.id,
          capture: failedCapture(draft.captureId, draft.generation, draft.createdAt, audio, failedDraft.transcript, error),
          jobs: [],
        });
      } catch (cause) {
  ...
  private async updateDraft(active: ActiveRecording, patch: Partial<Omit<RecordingDraftRecord, 'id' | 'captureId' | 'recoveryId' | 'createdAt'>>) {
    active.draft = { ...active.draft, ...patch, updatedAt: new Date().toISOString() };
    const snapshot = active.draft;
    const write = this.draftWrite.then(() => this.repositories.recordingDrafts.save(snapshot));
    this.draftWrite = write.catch(() => undefined);
    await write;
  }

  private persistLater(active: ActiveRecording, patch: Partial<Omit<RecordingDraftRecord, 'id' | 'captureId' | 'recoveryId' | 'createdAt'>>) {
    void this.updateDraft(active, patch).catch((error) => this.failPresentation(error));
  }
  ...
  private get repositories() {
    if (!this.processingDependencies) throw new Error('Capture storage has not been initialized.');
    return this.processingDependencies.repositories;
  }

  private get providers() {
    if (!this.processingDependencies) throw new Error('Capture providers have not been initialized.');
    return this.processingDependencies.providers;
  }

  private get secrets() {
    if (!this.processingDependencies) throw new Error('Capture secrets have not been initialized.');
    return this.processingDependencies.secrets;
  }

  private requireActive() {
    if (!this.active) throw domainError('conflict', 'recording', 'There is no active recording to control.');
    return this.active;
  }

  private failPresentation(error: unknown) {
    const detail = error instanceof DomainError ? error.detail : normalizeError(error, 'recording');
    this.publish({ capture: { ...this.state.capture, phase: detail.code === 'permission-denied' ? 'permission' : 'failure', message: detail.message, canRetry: detail.retryable } });
  }
...
function safeEndpoint(value: string | null) {
  ...
  try {
    ...
    return endpoint.protocol === 'https:' && !endpoint.username && !endpoint.password && !endpoint.search;
  } catch {
    return false;
  }
}

function liveMode(preferences: AppPreferencesRecord | null, providers: ProviderRegistryPort) {
  const provider = preferences ? providers.getSpeech(preferences.speechProvider.providerId) : null;
  return provider?.descriptor.capabilities['speech.streaming-transcription'] ? 'live' : 'after-saving';
}
...
function provisionalTranscript(active: ActiveRecording, text: string): TranscriptSnapshot {
...
function finalLiveTranscript(active: ActiveRecording, text: string, languageTag: string | null, segments: TranscriptSnapshot['segments']): TranscriptSnapshot | null {
...
function recoveryIdFromUri(uri: string) {
  const name = decodeURIComponent(uri).split('/').pop()?.replace(/\.wav$/iu, '');
...
function discoveredRecordingAudio() {
  ...
  try {
    draftAudio = inspectRecordingRecovery().map((candidate) => ({ ...candidate, location: 'draft' as const }));
  } catch {
  ...
  try {
    const promotedAudio = captureAudioDirectory()
      ...
      .map((file) => {
        try {
          return { uri: file.uri, audio: readRecoveredWav(file), error: null, location: 'capture' as const };
        } catch (error) {
          ...
          return { uri: file.uri, audio: null, error: normalizeError(error, 'recording'), location: 'capture' as const };
        }
      });
...
function candidateForDraft(draft: RecordingDraftRecord, candidates: readonly DiscoveredRecording[]) {
  ...
  return candidates.find((candidate) => candidate.location === 'capture'
    && candidate.audio
    && recoveryIdFromUri(candidate.uri) === draft.captureId)
    ?? candidates.find((candidate) => candidate.uri === draft.audio?.uri
...
function recoveredError(occurredAt: string): NormalizedError {
  return {
    ...
    operation: 'recording',
    message: 'We recovered this audio after recording was interrupted. Retry processing when ready.',
    retryable: true,
    occurredAt,
    providerId: null,
    statusCode: null,
...
function missingRecordingError(occurredAt: string): NormalizedError {
  return {
    code: 'storage-failed',
    operation: 'recording',
    message: 'The recording draft was recovered, but its audio file is missing. This failed draft remains available for recovery; record it again when ready.',
    retryable: true,
    occurredAt,
    providerId: null,
    statusCode: null,
...
function failedCapture(captureId: string, generation: DataGeneration, createdAt: string, audio: AudioAsset, transcript: TranscriptSnapshot | null, error: NormalizedError): CaptureRecord {
  const now = new Date().toISOString();
  return {
    id: captureId,
    generation,
...
export function useCapture() {
  const [state, setState] = useState<CaptureHomeState>(initialState);

  useEffect(() => {
    let unsubscribe: (() => void) | null = null;
    void controller().then(async (capture) => {
  ...
  const invoke = useCallback((action: (capture: CaptureController) => Promise<void>) => {
    void controller().then(action);
  }, []);
  ...
  return {
    ...
    retryCapture: (captureId: string) => invoke((capture) => capture.retryCapture(captureId)),
  };
}
