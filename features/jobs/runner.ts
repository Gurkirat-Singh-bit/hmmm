import type { LocalSubscriptionPort, NormalizedError } from '../domain/contracts';
import { domainError, normalizeError } from '../domain/errors';
import type { JobHandler, JobKind, JobRecord, JobRepository, JobRunnerPort } from './contracts';
...
export class ForegroundJobRunner implements JobRunnerPort {
  ...
  constructor(private readonly options: ForegroundJobRunnerOptions) {
    this.handlers = new Map(options.handlers.map((handler) => [handler.kind, handler] as const));
  ...
  start() {
    ...
    this.active = true;
    this.unsubscribe = this.options.subscriptions?.subscribe((change) => {
  ...
  private async recoverAndDrain() {
    const expired = await this.options.repository.requeueExpiredLeases(this.now().toISOString());
    for (const job of expired) {
      if (job.lastError) await this.options.onTerminalFailure?.(job, job.lastError);
    }
    await this.drain();
  }
  ...
  private begin(task: Promise<void>) {
    this.draining = task
      .catch(() => {
        if (this.active) this.timer = setTimeout(() => this.wake(), this.idlePollMs);
      })
  ...
  private async drain() {
    while (this.active) {
      ...
      if (!handler) {
        const error = domainError('unsupported', 'database', `No handler is registered for ${job.kind}.`).detail;
        await this.options.repository.fail(job.id, job.generation, this.now().toISOString(), error);
        await this.options.onTerminalFailure?.(job, error);
        continue;
      }
      try {
        await handler.run(job);
        await this.options.repository.succeed(job.id, job.generation, this.now().toISOString());
      } catch (cause) {
        const error = normalizeError(cause, job.kind === 'transcribe-capture' ? 'transcription' : 'report-generation');
