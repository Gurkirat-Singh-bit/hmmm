/**
 * Persisted domain snapshots and platform ports.
 *
 * These types intentionally contain no provider request shapes or secrets.
 */
...
export type RecordingDraftStatus = 'recording' | 'paused' | 'finalizing' | 'failed';
...
export type ReportField = 'gist' | 'evidence' | 'risks' | 'nextMove' | 'verdict';
...
export type ReportProvenance = Readonly<Record<ReportField, ReportFieldProvenance>>;
...
export interface SecretStorePort {
  ...
  deleteVersion(kind: CredentialKind, version: CredentialVersion): Promise<void>;
  clear(): Promise<void>;
}

export type RecordingPermission = 'undetermined' | 'denied' | 'granted';
export type RecordingSessionState = 'starting' | 'recording' | 'paused' | 'stopping' | 'stopped' | 'failed';

export type RecordingEvent =
  | Readonly<{ type: 'state'; state: RecordingSessionState }>
  | Readonly<{ type: 'duration'; durationMs: number }>
  | Readonly<{ type: 'audio-chunk'; data: Uint8Array; sequence: number; mimeType: string }>
...
export type ExportedCapture = Omit<CaptureRecord, 'audio' | 'generation'> & Readonly<{ hasSourceAudio: boolean }>;
...
export type ExportedJobHistory = Readonly<{
  ...
  revision: number;
  requestId: string;
  status: 'queued' | 'running' | 'retry-wait' | 'succeeded' | 'failed' | 'cancelled';
  attempts: number;
...
export type LocalDataChange = Readonly<{
  table: 'recording-drafts' | 'captures' | 'reports' | 'sources' | 'messages' | 'discussion-drafts' | 'jobs' | 'preferences';
}>;
