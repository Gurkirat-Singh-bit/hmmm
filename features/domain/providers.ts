...
export type LiveTranscriptEvent =
  | Readonly<{ type: 'transcript'; phase: 'provisional' | 'final'; text: string; sequence: number }>
...
export interface SpeechProviderPort {
  ...
  transcribe?: (context: ProviderContext, request: TranscriptionRequest) => Promise<FinalTranscript>;
...
export interface AiProviderPort {
  ...
  research?: (context: ProviderContext, request: ResearchRequest) => Promise<ResearchResult>;
  /** Present exactly when ai.report-generation is true. */
  generateReport?: (context: ProviderContext, request: ReportGenerationRequest) => Promise<GeneratedReport>;
  /** Present exactly when ai.discussion is true. */
  completeDiscussion?: (context: ProviderContext, request: DiscussionRequest) => Promise<DiscussionResponse>;
