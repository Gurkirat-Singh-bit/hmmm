...
export function useWeeklyActivity() {
  const [activity, setActivity] = useState<readonly DailyActivity[]>(() => emptyWeek().map(({ day, value }) => ({ day, value })));
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      if (live) setActivity(days.map(({ day, key }) => ({ day, value: counts.get(key) ?? 0 })));
    };
    void refresh().catch(() => undefined);
    void getVaultDatabase().then((database) => {
      const unsubscribe = database.subscriptions.subscribe((change) => {
        if (change.table === 'captures') void refresh().catch(() => undefined);
      });
      if (!live) unsubscribe();
