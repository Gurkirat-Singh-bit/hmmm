...
import { PROVIDER_RESPONSE_LIMITS, PROVIDER_TIMEOUT_MS } from '@/features/providers/config';
import { readJsonResponse } from '@/features/providers/transport';
...
async function requestCatalog(kind: CatalogKind, provider: ProviderDefinition, apiKey: string) {
  ...
  const timeout = setTimeout(() => controller.abort(), PROVIDER_TIMEOUT_MS.catalog);
  try {
    const response = await fetch(url, { headers: requestHeaders(provider, apiKey.trim()), signal: controller.signal });
    ...
    const models = parseCatalog(await readJsonResponse(response, provider.id, 'provider-configuration'), provider.id, kind);
    if (!models.length) throw new Error('No compatible models were returned. You can still enter an exact model ID.');
    return models;
  } catch (reason) {
    if (reason instanceof Error && reason.name === 'AbortError') throw new Error('The model catalog request timed out.');
    ...
    ].includes(reason.message)) throw reason;
    if (reason instanceof Error && reason.message.startsWith('Could not sync the model catalog (')) throw reason;
...
function requestHeaders(provider: ProviderDefinition, apiKey: string): Record<string, string> {
  ...
  if (provider.authentication === 'deepgram') return { Accept: 'application/json', Authorization: `Token ${apiKey}` };
...
function parseCatalog(payload: unknown, providerId: string, kind: CatalogKind) {
  ...
  const compatible = providerId === 'deepgram'
    ...
    : providerId === 'groq' && kind === 'ai'
      ? ids.filter((id) => !/audio|speech|transcri|whisper|tts|playai/i.test(id))
    : kind === 'speech'
      ? ids.filter((id) => /audio|speech|transcri|whisper/i.test(id))
      : ids.filter((id) => !/audio|speech|transcri|whisper|tts/i.test(id));
  const models = [...new Set(compatible)];
  if (models.length > PROVIDER_RESPONSE_LIMITS.catalogModels) throw new Error('Could not sync the model catalog. The provider returned too many models.');
...
function readObjectIds(value: unknown, property: string) {
  return boundedCatalogItems(value).flatMap((item) => {
    ...
    return item[property].length <= PROVIDER_RESPONSE_LIMITS.modelIdCharacters ? [item[property]] : [];
  });
}
...
function readGoogleIds(value: unknown) {
  return boundedCatalogItems(value).flatMap((item) => {
    ...
    if (typeof item.baseModelId === 'string') return item.baseModelId.length <= PROVIDER_RESPONSE_LIMITS.modelIdCharacters ? [item.baseModelId] : [];
...
function readOpenRouterTextIds(value: unknown) {
  return boundedCatalogItems(value).flatMap((item) => {
    ...
    if (!isRecord(item.architecture) || !Array.isArray(item.architecture.output_modalities)) return [item.id];
    return item.architecture.output_modalities.includes('text') ? [item.id] : [];
  });
}
...
function boundedCatalogItems(value: unknown) {
  ...
  if (value.length > PROVIDER_RESPONSE_LIMITS.catalogModels) {
    throw new Error('Could not sync the model catalog. The provider returned too many models.');
  }
  return value;
}
