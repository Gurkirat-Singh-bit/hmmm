...
import type { CaptureRecord, DataGeneration, NormalizedError, ReportUpdateProposal } from '@/features/domain/contracts';
...
export function useDiscussionHome() {
  const [state, setState] = useState<DiscussionHomeState>({ loading: true, captures: [], threads: [], error: null });
  ...
  useEffect(() => {
    ...
    void getVaultDatabase().then((database) => {
      ...
      unsubscribe = database.subscriptions.subscribe((change) => {
        if (change.table === 'captures' || change.table === 'messages') void load();
      });
      void load();
    }).catch((reason) => {
      if (active) setState((current) => ({ ...current, loading: false, error: normalizeError(reason, 'database') }));
    });
    return () => {
      active = false;
      unsubscribe?.();
...
export function useDiscussionThread(captureId: string | undefined) {
  ...
  const setComposer = useCallback((content: string) => {
  ...
  useEffect(() => {
    ...
    void getVaultDatabase().then((database) => {
      if (!active) return;
      unsubscribe = database.subscriptions.subscribe((change) => {
        if (change.table === 'captures' || change.table === 'reports' || change.table === 'sources' || change.table === 'messages' || change.table === 'discussion-drafts') {
          void load();
        }
      });
      void load();
  ...
  useEffect(() => {
    if (!captureId) return;
    return () => {
      void discussionService.abort(captureId, 'This reply was interrupted when you left the conversation.');
      const generation = selectedGeneration.current;
      if (generation === null) return;
      void draftWrites.current
        .then(() => discussionService.saveDraft(captureId, generation, composerRef.current))
        .catch(() => undefined);
    };
  }, [captureId]);
  ...
  useEffect(() => {
    ...
    const subscription = AppState.addEventListener('change', (nextState) => {
      if (nextState !== 'active') {
        void discussionService.abort(captureId, 'This reply was interrupted when the app moved to the background.');
      }
    });
  ...
  const send = useCallback(async () => {
    const generation = selectedGeneration.current;
    if (!captureId || generation === null || !composerRef.current.trim()) return;
    setNotice(null);
    try {
  ...
  const retry = useCallback(async (assistantId: string, mode: 'restart' | 'resume') => {
    const generation = selectedGeneration.current;
    if (!captureId || generation === null) return;
    setNotice(null);
    try {
  ...
  const sending = data?.messages.some((message) => message.role === 'assistant' && (message.status === 'queued' || message.status === 'streaming')) ?? false;
  return { data, loading, error, notice, composer, setComposer, sending, send, retry, applyProposal, refresh };
}
