...
function RootNavigator({ runtime }: { runtime: AppRuntimeSnapshot }) {
  ...
  return (
    <View style={styles.root}>
      <StatusBar
        backgroundColor={onboardingRoute ? colors.darkCanvas : colors.canvas}
        style={onboardingRoute ? "light" : "dark"}
      />
      <Stack
        screenOptions={({ route }) => ({
          animation: [
      ...
      {["/", "/vault", "/discuss", "/settings"].includes(pathname) ? (
        <FloatingBottomNav />
      ) : null}
      {showFeedback ? (
...
function RuntimeBootstrap({
  ...
  return (
    <View style={styles.bootstrap}>
      ...
      <Text
        accessibilityRole={loading ? "progressbar" : "header"}
        style={styles.bootstrapTitle}
      >
        {loading
          ? "Preparing your local workspace…"
          : "Hmmmidea needs a restart"}
      </Text>
      <Text style={styles.bootstrapBody}>
        {error ?? "Your ideas stay on this device while Hmmmidea gets ready."}
      </Text>
      {!loading ? (
        <Pressable
          accessibilityRole="button"
          accessibilityLabel="Retry local workspace setup"
          onPress={() => void retryAppRuntime().catch(() => undefined)}
