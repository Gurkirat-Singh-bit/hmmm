 ...
 * @description Creates, shares, and reconciles non-secret JSON and PDF export artifacts.
...
function requireAndroid() {
  if (Platform.OS !== "android") {
    throw domainError(
      "unsupported",
      "export",
      "File exports are available on Android only.",
    );
  }
}
...
function artifactStamp() {
  ...
  const random =
    globalThis.crypto?.randomUUID?.().replace(/[^a-z0-9]/giu, "") ??
...
function citationLink(source: SourceRecord) {
  ...
  if (!url)
    return `<span class="citation">${html(source.title || source.domain)}</span>`;
...
export function buildIdeaPdfHtml({
  capture,
  report,
  sources,
}: Omit<PdfExportRequest, "format">) {
  const sourcesById = new Map(
    sources
      .filter(
        (source) =>
          source.captureId === capture.id &&
          source.reportRevision === report.revision,
      )
      .map((source) => [source.id, source]),
  );
  const evidence = report.content.evidence.length
    ? `<ul>${report.content.evidence
        .map((item) => {
...
function sanitizeNonSecretBundle(
  bundle: NonSecretExportBundle,
): NonSecretExportBundle {
  return {
    ...bundle,
    preferences: {
...
export async function shareExportArtifact(artifact: ExportArtifact) {
  ...
  if (!new File(artifact.uri).exists) {
    throw domainError(
      "not-found",
      "export",
      "The temporary export file is no longer available.",
    );
  }
  ...
  await Sharing.shareAsync(artifact.uri, {
    dialogTitle: "Share Hmmmidea export",
...
export async function shareIdeaPdfExport(
...
export const androidExportPort = {
  create: createExportArtifact,
} satisfies ExportPort;
export const androidSharePort = {
  isAvailable: async () =>
    Platform.OS === "android" && Sharing.isAvailableAsync(),
  share: async (artifacts) => {
    if (artifacts.length !== 1) {
      throw domainError(
        "unsupported",
        "export",
        "Android shares one export file at a time.",
      );
    }
