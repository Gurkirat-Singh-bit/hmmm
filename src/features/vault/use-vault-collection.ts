/**
 * @file use-vault-collection.ts
 * @description React state for Vault queries, selection, deletion, starring, and export.
...
export function useVaultCollection() {
  ...
  const [state, setState] = useState<VaultCollectionState>({
    loading: true,
    captures: [],
    error: null,
  });
    ...
    void getVaultDatabase()
      .then((database) => {
        ...
        unsubscribe = database.subscriptions.subscribe((change) => {
          if (change.table === "captures" || change.table === "reports")
            void load();
        });
        void load();
      })
