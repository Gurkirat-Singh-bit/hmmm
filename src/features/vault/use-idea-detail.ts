/**
 * @file use-idea-detail.ts
 * @description React state and actions for loading, editing, retrying, and deleting one idea.
...
export function useIdeaDetail(
  ...
  selectedRevision: ReportRevision | null,
) {
  const [state, setState] = useState<IdeaDetailState>({
    loading: true,
    data: null,
    error: null,
  });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const [activeReport, provisional, revisions, jobs] = await Promise.all([
          ...
          database.repositories.jobs.listForCapture(captureId),
        ]);
        const report =
        ...
        const sources = report
          ? await database.repositories.reports.listSources(
              captureId,
              report.revision,
            )
          : [];
        if (active)
          setState({
            ...
            data: {
              ...
              sources,
              jobs,
              isProvisional: report?.phase === "provisional",
            },
            error: null,
          });
      } catch (error) {
