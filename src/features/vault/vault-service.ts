 ...
 * @description Implements repository-backed Vault queries, edits, revisions, deletion, and export.
...
function sourcesForRevision(sources: readonly SourceRecord[]) {
  return sources.map(
    ({ captureId: _captureId, reportRevision: _reportRevision, ...source }) =>
      source,
  );
}
...
function userProvenance(
  ...
  changedAt: string,
  origin: "user-edited" | "discussion-update",
): ReportProvenance {
...
function requireEditable(
  ...
  if (!report || capture.generation !== report.generation) {
    throw domainError(
      "cancelled",
      "report-generation",
      "This report is outdated because the idea was deleted or replaced.",
    );
  }
  if (
    report.phase !== "final" ||
    capture.activeReportRevision !== report.revision
  ) {
    throw domainError(
      "conflict",
      "report-generation",
      "Open the active ready report before updating it.",
    );
  }
  if (capture.transcript?.phase !== "final") {
    throw domainError(
      "conflict",
      "report-generation",
      "A final transcript is required before updating this report.",
    );
  }
}
...
export async function applyDiscussionReportUpdate(
  ...
  if (
    proposal.captureId !== report.captureId ||
    ...
    throw domainError(
      "conflict",
      "discussion",
      "The discussion proposal is no longer based on this report.",
    );
  }
  const database = await getVaultDatabase();
  ...
  if (capture.generation !== expectedGeneration) {
    throw domainError(
      "conflict",
      "discussion",
      "The discussion proposal is no longer based on this idea.",
    );
  }
  requireEditable(capture, report);
  const now = new Date().toISOString();
  return database.repositories.reports.appendRevision({
    ...
    provenance: userProvenance(
      report,
      changedFields(report.content, proposal.content),
      now,
      "discussion-update",
    ),
    explicitlyReplacedUserFields: [],
...
export async function regenerateReport(
  ...
  if (current.generation !== capture.generation) {
    throw domainError(
      "cancelled",
      "report-generation",
      "This idea was deleted or replaced. Refresh the report.",
    );
  }
  if (current.activeReportRevision !== expectedActiveRevision) {
    throw domainError(
      "conflict",
      "report-generation",
      "The report changed before regeneration was confirmed. Refresh the idea and review the current revision.",
    );
  }
  if (current.transcript?.phase !== "final") {
    throw domainError(
      "conflict",
      "report-generation",
      "A final transcript is required before regenerating this report.",
    );
  }
  const now = new Date().toISOString();
...
export async function setCaptureStarred(
  capture: Pick<CaptureRecord, "id" | "generation">,
  starred: boolean,
) {
  const database = await getVaultDatabase();
  ...
  if (!current)
    throw domainError("not-found", "database", "The capture was not found.");
  if (current.generation !== capture.generation) {
    throw domainError(
      "cancelled",
      "database",
      "This idea was deleted or replaced. Refresh the Vault.",
    );
  }
  await database.repositories.captures.setStarred(
    capture.id,
    starred,
...
export async function renameCapture(
  ...
  await database.repositories.captures.setTitle(
    capture.id,
    title,
    new Date().toISOString(),
    capture.generation,
...
export async function deleteCaptures(
  captures: readonly Pick<CaptureRecord, "id" | "generation">[],
) {
  const database = await getVaultDatabase();
  const current = await Promise.all(
    captures.map((capture) => database.repositories.captures.get(capture.id)),
  );
  if (
    current.some(
      (capture, index) =>
        !capture || capture.generation !== captures[index].generation,
    )
  ) {
    throw domainError(
      "cancelled",
      "database",
      "One or more selected ideas were deleted or replaced. Refresh the Vault.",
    );
  }
...
export async function shareCaptures(captureIds: readonly CaptureId[]) {
  ...
  const captures = (
    await Promise.all(
      captureIds.map((id) => database.repositories.captures.get(id)),
    )
  ...
  if (!captures.length)
    throw domainError(
      "not-found",
      "export",
      "The selected ideas are no longer in the Vault.",
    );
  const reports = await Promise.all(
    captures.map((capture) =>
      database.repositories.reports.getActive(capture.id),
    ),
  );
  await Share.share({
    title:
      captures.length === 1
        ? (captures[0].title ?? "Hmmmidea")
        : "Hmmmidea ideas",
    message: captures
      .map((capture, index) => shareText(capture, reports[index]))
      .join("\n\n────────\n\n"),
  });
}
...
export async function openCitation(url: string, domain: string) {
  ...
  try {
    ...
    throw domainError(
      "invalid-url",
      "export",
      "This source link is not a valid URL.",
    );
  }
  ...
  if (
    ...
    throw domainError(
      "invalid-url",
      "export",
      "Source links must use credential-free HTTPS.",
    );
  }
  if (!(await Linking.canOpenURL(parsed.toString()))) {
    throw domainError(
      "unsupported",
      "export",
      "This device cannot open the source link.",
    );
  }
