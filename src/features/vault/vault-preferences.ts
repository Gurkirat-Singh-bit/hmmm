/**
 * @file vault-preferences.ts
 * @description Persists non-secret Vault filter and sort preferences.
...
import type { CaptureSort } from "@/features/storage/contracts";
import type { CaptureStatus } from "@/features/domain/contracts";

const key = "hmmm.vault-filters.v1";
const statuses: readonly CaptureStatus[] = [
  "queued",
  "transcribing",
  "naming",
  "researching",
...
const sorts: readonly CaptureSort[] = [
  "newest",
  "oldest",
  "title-asc",
  "title-desc",
...
function valid(value: unknown): VaultFilters {
  ...
  return {
    ...
    statuses: Array.isArray(candidate.statuses)
      ? candidate.statuses.filter((status): status is CaptureStatus =>
          statuses.includes(status as CaptureStatus),
        )
      : [],
    sort: sorts.includes(candidate.sort as CaptureSort)
      ? (candidate.sort as CaptureSort)
      : "newest",
  };
}
