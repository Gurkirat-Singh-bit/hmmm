/**
 * @file connection.ts
 * @description Serializes SQLite writes and exposes transaction and change-subscription primitives.
...
export class SqliteStore {
  ...
  write<Result>(
    task: (transaction: SQLiteDatabase) => Promise<Result>,
  ): Promise<Result> {
    if (this.closePromise)
      return Promise.reject(new Error("The SQLite store is closing."));
  ...
  read<Result>(
    task: (database: SQLiteDatabase) => Promise<Result>,
  ): Promise<Result> {
    if (this.closePromise)
      return Promise.reject(new Error("The SQLite store is closing."));
  ...
  close(): Promise<void> {
    if (!this.closePromise)
      this.closePromise = this.writeTail.then(() => this.database.closeAsync());
...
class SqliteSubscriptions implements LocalSubscriptionPort {
  ...
  subscribe(listener: (change: LocalDataChange) => void) {
    const subscription = addDatabaseChangeListener((event) => {
      ...
      if (table && event.databaseFilePath === this.database.databasePath)
        listener({ table });
    });
