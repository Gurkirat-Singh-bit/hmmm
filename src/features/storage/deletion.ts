/**
 * @file deletion.ts
 * @description Runs durable structured-data deletion followed by retryable audio cleanup.
...
export class LocalDataDeletion implements DataDeletionPort {
  ...
  async execute(request: DeletionRequest) {
    ...
    if (request.target.kind === "full-reset")
      await this.clearSecrets(request.operationId);
  ...
  async retry(operationId: string): Promise<DeletionReceipt> {
    const tombstone =
      await this.dependencies.deletions.getTombstone(operationId);
    if (!tombstone)
      throw domainError(
        "not-found",
        "database",
    ...
    await this.dependencies.cleanup.requeueFailed(
      operationId,
      this.now().toISOString(),
    );
    if (tombstone.target.kind === "full-reset")
      await this.clearSecrets(operationId);
    await this.drainCleanup();
  ...
  private async clearSecrets(operationId: string) {
    ...
    try {
      ...
      await this.dependencies.deletions.setSecureData(
        operationId,
        "deleted",
        now,
      );
  ...
  private async drainCleanup() {
    while (true) {
      ...
      try {
        if (await this.dependencies.audioFiles.exists(item.uri))
          await this.dependencies.audioFiles.delete(item.uri);
        ...
        const delayMs = Math.min(
          CLEANUP_RULES.retryMaxDelayMs,
          CLEANUP_RULES.retryBaseDelayMs * 2 ** Math.max(0, item.attempts - 1),
        );
        await this.dependencies.cleanup.retry(
          item.id,
