/**
 * @file live-transcription.ts
 * @description Streams recording chunks without allowing provider failures to interrupt local capture.
...
import { LIVE_TRANSCRIPTION } from "../config";
...
export class RecordingLiveTranscription {
  ...
  start(): void {
    ...
    this.recordingUnsubscribe = this.source.subscribe((event) =>
      this.onRecordingEvent(event),
    );
      ...
      try {
        ...
        const live = await Promise.race([
          ...
          new Promise<null>((resolve) =>
            setTimeout(() => resolve(null), LIVE_TRANSCRIPTION.openTimeoutMs),
          ),
        ]);
        if (!live) {
          void candidate.then(
            (late) => this.cancelQuietly(late),
            () => undefined,
          );
          this.publishClosed();
          this.scheduleReconnect();
  ...
  private onLiveEvent(
    owner: LiveTranscriptionSessionPort,
    ...
    const reconnectDuplicate =
      event.phase === "final" &&
      this.firstFinalAfterReconnect &&
      text === this.lastFinal;
    if (event.phase === "final") this.firstFinalAfterReconnect = false;
    if (
      this.seenKeys.has(key) ||
      reconnectDuplicate ||
      (event.phase === "provisional" && text === this.lastProvisional)
    )
      return;
    this.remember(key);
  ...
  private remember(key: string): void {
    ...
    if (this.seenOrder.length <= LIVE_TRANSCRIPTION.transcriptDedupeWindow)
      return;
  ...
  private send(
    live: LiveTranscriptionSessionPort,
    chunk: QueuedChunk,
  ): Promise<boolean> {
    return Promise.race([
      ...
      new Promise<false>((resolve) =>
        setTimeout(() => resolve(false), LIVE_TRANSCRIPTION.sendTimeoutMs),
      ),
    ]);
  }
  ...
  private async waitForDrain(): Promise<boolean> {
    ...
    return Promise.race([
      ...
      new Promise<false>((resolve) =>
        setTimeout(
          () => resolve(false),
          LIVE_TRANSCRIPTION.finishDrainTimeoutMs,
        ),
      ),
    ]);
  }
