 ...
 * @description Owns recorded-audio paths, validation, preparation, and cleanup inside app storage.
...
import { AUDIO_DIRECTORIES, RECORDING_AUDIO } from "../config";
...
function safeId(value: string, label: string): string {
  if (!SAFE_ID.test(value)) {
    throw domainError(
      "unsupported",
      "recording",
      `${label} is not a valid app-owned audio identifier.`,
    );
  }
  return value;
}
...
function draftDirectoryPath(): Directory {
  return new Directory(
    Paths.document,
    AUDIO_DIRECTORIES.root,
    AUDIO_DIRECTORIES.drafts,
  );
}
...
function uploadDirectory(requestId: string): Directory {
  return ensureDirectory(
    new Directory(
      Paths.cache,
      AUDIO_DIRECTORIES.uploads,
      safeId(requestId, "Upload request ID"),
    ),
  );
}
export function recordingDraftFile(
  recoveryId: string,
  container = "wav",
): File {
  if (!/^[a-z0-9]{1,10}$/u.test(container)) {
    throw domainError(
      "unsupported",
      "recording",
      "Recording container is invalid.",
    );
  }
  return new File(
    recordingDraftDirectory(),
    `${safeId(recoveryId, "Recording recovery ID")}.${container}`,
  );
}
...
function appOwnedAudioFile(uri: string): File {
  if (!isAppOwnedAudioUri(uri)) {
    throw domainError(
      "unsupported",
      "recording",
      "The audio file is outside app-owned storage.",
    );
  }
  ...
  async listAppOwnedAudioUris(): Promise<readonly string[]> {
    ...
    for (const directory of appOwnedDirectories()) {
      ...
      for (const entry of directory.list()) {
        if (entry instanceof File && isAppOwnedAudioUri(entry.uri))
          uris.add(entry.uri);
      }
    }
    return [...uris];
...
function positive(value: number, label: string): number {
  if (!Number.isFinite(value) || value <= 0) {
    throw domainError(
      "storage-failed",
      "recording",
      `The audio file has invalid ${label} metadata.`,
    );
  }
  return value;
}
export function validateAudioAsset(audio: AudioAsset): AudioAsset {
  ...
  if (
    !/^[a-z0-9]{1,10}$/u.test(audio.container) ||
    !audio.mimeType.startsWith("audio/")
  ) {
    throw domainError(
      "storage-failed",
      "recording",
      "The audio file has invalid format metadata.",
    );
  }
  if (file.size < WAV_HEADER_BYTES || audio.byteLength !== file.size) {
    throw domainError(
      "storage-failed",
      "recording",
      "The audio file size does not match its metadata.",
    );
  }
  if (audio.container === "wav") {
    ...
    if (
      ...
      throw domainError(
        "storage-failed",
        "recording",
        "The audio metadata does not match the WAV file.",
      );
    }
  }
  return audio;
...
export function readRecoveredWav(file: File): AudioAsset {
  ...
  if (
    ...
    throw domainError(
      "storage-failed",
      "recording",
      "A recovered recording is not a valid PCM WAV file.",
    );
  }
  ...
  if (
    ...
    throw domainError(
      "storage-failed",
      "recording",
      "A recovered recording has inconsistent WAV metadata.",
    );
  }
...
export function listRecoverableAudio(): readonly AudioAsset[] {
  return inspectRecordingRecovery().flatMap((candidate) =>
...
export function promoteRecordingAudio(
  audio: AudioAsset,
  captureId: string,
): AudioAsset {
  if (!/^[a-z0-9]{1,10}$/u.test(audio.container)) {
    throw domainError(
      "storage-failed",
      "recording",
      "The audio file has invalid container metadata.",
    );
  }
  const destination = new File(
    captureAudioDirectory(),
    `${safeId(captureId, "Capture ID")}.${audio.container}`,
  );
  ...
  if (!isInside(source.uri, recordingDraftDirectory())) {
    if (source.uri !== destination.uri) {
      throw domainError(
        "conflict",
        "recording",
        "Source audio is already retained for another capture.",
      );
    }
    return audio;
  }
  if (destination.exists) {
    throw domainError(
      "conflict",
      "recording",
      "Source audio already exists for this capture.",
    );
  }
...
function formatFromAudio(audio: AudioAsset): "wav" | "aac" | "opus" | null {
  ...
  if (
    audio.container === "aac" ||
    audio.container === "m4a" ||
    audio.mimeType.includes("aac")
  )
    return "aac";
  if (audio.container === "opus" || audio.mimeType.includes("opus"))
    return "opus";
  return null;
}
function mimeForTrimResult(result: TrimAudioResult): string {
  return (
    result.mimeType ||
    (result.compression?.format === "opus" ? "audio/opus" : "audio/aac")
  );
}
function assetFromTrimResult(
  result: TrimAudioResult,
  destination: File,
): AudioAsset {
  const bitRateBps =
    result.compression?.bitrate ??
    result.sampleRate * result.channels * result.bitDepth;
  return {
    uri: destination.uri,
...
function partDuration(
  ...
  const byBytes =
    limits.maxBytes == null
      ? audio.durationMs
      : Math.floor((limits.maxBytes * 8 * 1_000 * 0.9) / targetBitRateBps);
  const duration = Math.floor(Math.min(audio.durationMs, byDuration, byBytes));
  if (duration < 1) {
    throw domainError(
      "provider-rejected",
      "transcription",
      "The speech provider audio limit is too small.",
    );
  }
  return duration;
}
export async function prepareProviderAudioUpload(
  ...
  const mimeAccepted =
    ...
    limits.acceptedMimeTypes.includes(audio.mimeType);
  const tooLarge =
  ...
  const formatAdjustment =
    (limits.preferredFormat != null &&
      limits.preferredFormat !== sourceFormat) ||
    (limits.sampleRateHz != null &&
      limits.sampleRateHz !== audio.sampleRateHz) ||
    (limits.channelCount != null &&
      limits.channelCount !== audio.channelCount) ||
  ...
  const targetFormat =
    limits.preferredFormat ?? (mimeAccepted ? sourceFormat : null);
  if (!targetFormat) {
    throw domainError(
      "unsupported",
      "transcription",
      "The source audio cannot be converted to a provider-supported format.",
    );
  }
  ...
  try {
    ...
    throw domainError(
      "storage-failed",
      "file-cleanup",
      "Stale provider audio could not be removed.",
      true,
    );
  }
  const targetBitRateBps =
    limits.bitRateBps ?? (targetFormat === "wav" ? audio.bitRateBps : 64_000);
  ...
  const cleanup = async () => {
    ...
    if (cleanupFailed)
      throw domainError(
        "storage-failed",
        "file-cleanup",
        "Temporary provider audio could not be fully removed.",
        true,
      );
  };
  ...
  try {
    ...
    for (
      ...
      startMs < audio.durationMs;
      startMs += durationPerPart, index += 1
    ) {
      ...
      for (const extension of ["wav", "aac", "opus"]) {
        temporaryFiles.push(
          new File(Paths.document, `${baseName}.${extension}`),
        );
      }
      ...
      if (limits.maxBytes != null && part.byteLength > limits.maxBytes) {
        throw domainError(
          "provider-rejected",
          "transcription",
          "A prepared audio part exceeds the speech provider size limit.",
        );
      }
      if (
        limits.maxDurationMs != null &&
        part.durationMs > limits.maxDurationMs + 50
      ) {
        throw domainError(
          "provider-rejected",
          "transcription",
          "A prepared audio part exceeds the speech provider duration limit.",
        );
      }
      if (
        limits.acceptedMimeTypes.length > 0 &&
        !limits.acceptedMimeTypes.includes(part.mimeType)
      ) {
        throw domainError(
          "unsupported",
          "transcription",
          "The converted audio format is not accepted by the speech provider.",
        );
      }
      parts.push(part);
    }
