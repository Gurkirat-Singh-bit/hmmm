/**
 * @file playback.ts
 * @description Expo Audio adapter for playback of retained source recordings.
...
import { PLAYBACK_UPDATE_INTERVAL_MS } from "../config";
...
class ExpoPlaybackSession implements PlaybackSessionPort {
  ...
  async seek(positionMs: number): Promise<void> {
    ...
    if (!Number.isFinite(positionMs)) {
      throw domainError(
        "unsupported",
        "recording",
        "Playback position must be a finite number.",
      );
    }
    ...
    try {
      ...
      if (this.state === "ended" && targetMs < this.durationMs)
        this.setState("paused");
    } catch (error) {
  ...
  private onStatus(status: PlayerStatus): void {
    ...
    if (status.duration > 0)
      this.durationMs = Math.round(status.duration * 1_000);
    this.emit({
      type: "position",
  ...
  private requireActive(): void {
    if (this.disposed)
      throw domainError(
        "conflict",
        "recording",
        "The playback session has already been unloaded.",
      );
  }
  ...
  async open(audio: AudioAsset): Promise<PlaybackSessionPort> {
    if (Platform.OS !== "android") {
      throw domainError(
        "unsupported",
        "recording",
        "Source audio playback is supported on Android only.",
      );
    }
