/**
 * @file use-notification-settings.ts
 * @description React state for notification permission and preference changes.
...
export function useNotificationSettings() {
    ...
    try {
      if (enabled) {
        ...
        if (permission !== "granted" && permission !== "provisional") {
          setMessage(
            "Android notifications are off. Allow them in system settings to enable background completion alerts.",
          );
          return;
        }
      }
      ...
      setMessage(
        enabled
          ? "Background completion alerts are on."
          : "Background completion alerts are off.",
      );
    } catch {
      setMessage("Could not update notification settings.");
    } finally {
  ...
  const setCategory = (
    category: "reportReady" | "processingFailed",
    enabled: boolean,
  ) =>
    save(
      { ...preferences, [category]: enabled },
      enabled
        ? "That background outcome will be announced."
        : "That background outcome will stay quiet.",
    );
