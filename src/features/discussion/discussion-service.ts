 ...
 * @description Persists discussion turns, streams provider replies, and proposes report revisions.
 * @author Gurkirat Singh
 * @license MIT
 */

import type { AppDatabase } from "@/features/storage/database";
import type {
...
import type {
  DiscussionRequest,
  ProviderContext,
} from "@/features/domain/providers";
import {
  providerCredentials,
  readPreferences,
} from "@/features/onboarding/storage";
import { PROVIDER_CONTEXT_LIMITS } from "@/features/provider/config";
...
class DiscussionService {
  ...
  async loadHome() {
    ...
    await Promise.all(
      captures.map((capture) =>
        this.reconcileInterrupted(database, capture.id, capture.generation),
      ),
    );
    const threadEntries = await Promise.all(
      captures.map(async (capture) => {
        const messages = await database.repositories.messages.list(
          capture.id,
          1,
          null,
        );
    ...
    const threads = threadEntries
      .filter((thread): thread is DiscussionThreadSummary => thread !== null)
      .sort((left, right) => right.updatedAt.localeCompare(left.updatedAt));
    const started = new Set(threads.map((thread) => thread.captureId));
    return {
      captures: captures.filter(
        (capture) => capture.status === "ready" && !started.has(capture.id),
      ),
      threads,
    };
  }
  async loadThread(captureId: string): Promise<DiscussionThreadData | null> {
    ...
    if (!capture) {
      await this.abort(
        captureId,
        "This reply stopped because the idea was deleted.",
      );
      return null;
    }
    ...
    if (!current || current.generation !== capture.generation) {
      await this.abort(
        captureId,
        "This reply stopped because the idea was deleted or replaced.",
      );
      return null;
    }
    ...
    return {
      ...
      messages,
      draft,
      availability: discussionAvailability(preferences),
    };
  }
  async saveDraft(
    captureId: CaptureId,
    ...
    try {
      if (!content.trim()) {
        await database.repositories.discussionDrafts.delete(
          captureId,
          expectedGeneration,
        );
        return;
      }
      await database.repositories.discussionDrafts.save({
        captureId,
    ...
    try {
      ...
      if (availability !== "ready") {
        throw domainError(
          ...
          "discussion",
          "Add a working AI provider before sending a message.",
          true,
        );
      }
      ...
      const { assistant } =
        await database.repositories.messages.appendUserAndStartAssistant({
          id: identity("message"),
  ...
  async retry(
    ...
    if (this.activeStreams.has(captureId) || this.pendingTurns.has(captureId)) {
      throw domainError(
        "conflict",
        "discussion",
        "Wait for the current reply before trying again.",
      );
    }
    this.pendingTurns.add(captureId);
    try {
      ...
      if (availability !== "ready") {
        throw domainError(
          "configuration-missing",
          "discussion",
          "Add a working AI provider before retrying.",
          true,
        );
      }
      const existing = (
        await database.repositories.messages.list(
          captureId,
          MESSAGE_PAGE_SIZE,
          null,
        )
      ...
      if (!existing)
        throw domainError(
          "not-found",
          "discussion",
          "This saved reply is no longer available.",
        );
      const assistant = await database.repositories.messages.retryAssistant(
        assistantId,
        expectedGeneration,
        mode,
  ...
  async applyProposal(
    ...
    const database = await getVaultDatabase();
    await this.captureForGeneration(
      database,
      proposal.captureId,
      expectedGeneration,
    );
    try {
      const activeReport = await database.repositories.reports.getActive(
        proposal.captureId,
      );
      if (
        !activeReport ||
        activeReport.generation !== expectedGeneration ||
        activeReport.revision !== proposal.baseRevision
      ) {
        throw domainError(
          "conflict",
          "discussion",
          "This proposal is based on an older report. Review the latest report before applying it.",
        );
      }
      const sources = await database.repositories.reports.listSources(
        proposal.captureId,
        activeReport.revision,
      );
      return applyDiscussionReportUpdate(
        proposal,
        activeReport,
        sources,
        expectedGeneration,
  ...
  async abort(captureId: CaptureId, message: string) {
    ...
    await active.database.repositories.messages
      .interruptAssistant(
        active.assistantId,
        active.generation,
  ...
  private async stream(
    ...
    try {
      ...
      const provider = providerRegistry.getAi(
      ...
      if (!provider) {
        throw domainError(
          "configuration-missing",
          "discussion",
          "The selected AI provider does not support discussion.",
          true,
        );
      }
      let completed = false;
      if (
        !provider.descriptor.capabilities["ai.discussion-streaming"] ||
        !provider.streamDiscussion
      ) {
        ...
        let pendingDelta = "";
        let persistedSequence = assistant.lastSequence;
        let receivedFirstDelta = false;
        const flushDelta = async () => {
          if (!pendingDelta) return;
          const delta = pendingDelta;
          const nextSequence = persistedSequence + 1;
          await active.database.repositories.messages.appendAssistantDelta(
            assistant.id,
            active.generation,
            nextSequence,
          ...
          pendingDelta = pendingDelta.slice(delta.length);
          persistedSequence = nextSequence;
          receivedFirstDelta = true;
        };
        try {
          for await (const event of provider.streamDiscussion(
            context,
            request,
          )) {
            if (active.controller.signal.aborted)
            ...
            if (event.type === "delta") {
              pendingDelta += event.content;
              if (
                !receivedFirstDelta ||
                pendingDelta.length >= STREAM_WRITE_CHARACTERS
              )
                await flushDelta();
            } else {
              await flushDelta();
      ...
      if (!completed)
        throw domainError(
          ...
          "discussion",
          "The provider ended before completing the reply.",
          true,
        );
    } catch (error) {
      if (
        await this.captureWasDeleted(
          active.database,
          captureId,
          active.generation,
        )
        ...
        active.controller.abort();
        if (this.activeStreams.get(captureId) === active)
          this.activeStreams.delete(captureId);
  ...
  private async requestFor(
    ...
    if (!capture || capture.generation !== assistant.generation)
      throw deletedCaptureError();
    const user = messages.find(
      (message) =>
        message.id === assistant.replyToMessageId &&
        message.role === "user" &&
        message.generation === assistant.generation,
    );
    if (!user)
      throw domainError(
        "not-found",
        "discussion",
        "The message being answered is no longer available.",
      );
    const report =
      user.reportRevision === null
        ? null
        : await database.repositories.reports.get(
            captureId,
            user.reportRevision,
          );
    if (report && report.generation !== assistant.generation)
      throw deletedCaptureError();
    return {
    ...
    await Promise.all(
      messages
        ...
        .map(async (message) => {
          try {
            const assistant =
              await database.repositories.messages.startAssistant({
                id: identity("message"),
                captureId,
                expectedGeneration,
            ...
            await database.repositories.messages.interruptAssistant(
              ...
              expectedGeneration,
              cancellationError(
                "This reply was interrupted before the app could finish it.",
              ),
              new Date().toISOString(),
            );
          } catch (error) {
  ...
  private async captureForGeneration(
    database: AppDatabase,
    captureId: CaptureId,
    expectedGeneration: DataGeneration,
  ) {
  ...
  private async captureWasDeleted(
    database: AppDatabase,
    captureId: CaptureId,
    expectedGeneration: DataGeneration,
  ) {
    const capture = await database.repositories.captures
      .get(captureId)
      .catch(() => null);
  ...
  return [
    {
      ...
      content: assistant.content.slice(
        0,
    ...
    },
    {
      ...
      role: "user" as const,
      content:
        "Continue the saved partial reply from this point. Return only new continuation text and do not repeat the saved partial reply.",
    },
  ] as const;
}
