...
import { filterModelCatalog } from "@/features/provider/model-discovery";

/** Renders a searchable model catalog with an explicit exact-ID action. */
export function SearchableModelPicker({
  ...
  const filtered = useMemo(
    () => filterModelCatalog(options, cleanQuery),
    [cleanQuery, options],
  );
  const custom = Boolean(
    cleanQuery &&
    !options.some(
      (option) => option.toLowerCase() === cleanQuery.toLowerCase(),
    ),
  );
    ...
    <>
      <View style={styles.group}>
        <View style={styles.labelRow}>
          ...
          {onRefresh ? (
            <Pressable
              accessibilityHint="Fetches the provider’s current model list."
              accessibilityLabel="Sync model catalog"
        ...
        <Pressable
          accessibilityHint="Opens a searchable list of provider model IDs."
          accessibilityLabel={`${label}: ${value || "no model selected"}`}
          accessibilityRole="button"
      ...
      >
        <SafeAreaView accessibilityViewIsModal style={styles.modal}>
          ...
          <View style={styles.search}>
            ...
            <TextInput
              accessibilityHint="Type to filter models or enter an exact model ID."
              accessibilityLabel="Search models"
              autoCapitalize="none"
              autoCorrect={false}
              autoFocus
              onChangeText={setQuery}
              placeholder="Search model or paste exact ID"
          ...
          <FlatList
            ...
            ListHeaderComponent={
              custom ? (
                <Pressable
                  accessibilityHint="Uses this exact provider model identifier."
                  accessibilityLabel={`Use exact model ID ${cleanQuery}`}
                  accessibilityRole="button"
                  onPress={() => choose(cleanQuery)}
                  style={({ pressed }) => [
                    styles.customChoice,
                    pressed && styles.pressed,
                  ]}
                >
                  <Text style={styles.customChoiceLabel}>
                    USE EXACT MODEL ID
                  </Text>
                  <Text numberOfLines={2} style={styles.customChoiceValue}>
                    {cleanQuery}
                  </Text>
                </Pressable>
              ) : (
                <Text style={styles.resultCount}>
                  {visibleOptions.length} model
              ...
              custom ? null : (
                <View style={styles.empty}>
                  ...
                  <Text style={styles.emptyTitle}>
                    {loading ? "Loading models…" : "No matching model."}
                  </Text>
                  <Text style={styles.emptyBody}>
                    {error ??
                      "Type or paste an exact model ID above to use it."}
                  </Text>
                </View>
              )
            }
              ...
              return (
                ...
                >
                  <View style={styles.rowCopy}>
                    ...
                    {selected ? (
                      <Text style={styles.custom}>SELECTED MODEL</Text>
                    ) : null}
                  </View>
                  {selected ? (
                    <Check color={colors.ink} size={18} weight="bold" />
                  ) : null}
                </Pressable>
              );
            }}
...
const styles = StyleSheet.create({
  ...
  placeholder: { color: colors.darkMuted },
  hint: {
    color: colors.darkMuted,
  ...
  empty: { gap: 6, paddingVertical: 36 },
  emptyTitle: {
    color: colors.ink,
  ...
  triggerLight: { borderColor: colors.line, backgroundColor: colors.canvas },
