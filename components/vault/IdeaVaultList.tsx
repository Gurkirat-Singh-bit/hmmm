...
export function IdeaVaultList({ ideas, onOpen, onToggleStar }: { ideas: readonly VaultIdea[]; onOpen(id: string): void; onToggleStar(id: string): void }) {
  return <View style={styles.list}>{ideas.map((idea) => <IdeaVaultRow idea={idea} key={idea.id} onOpen={onOpen} onToggleStar={onToggleStar} />)}{ideas.length === 0 ? <View style={styles.empty}><Text style={styles.emptyTitle}>No ideas found.</Text><Text style={styles.emptyBody}>Try another search or filter.</Text></View> : null}</View>;
}

export function IdeaVaultRow({ idea, onOpen, onToggleStar }: { idea: VaultIdea; onOpen(id: string): void; onToggleStar?(id: string): void }) {
  return <Pressable accessibilityRole="button" onPress={() => onOpen(idea.id)} style={({ pressed }) => [styles.row, { backgroundColor: accents[idea.accent] }, pressed && styles.pressed]}><View style={styles.copy}><Text numberOfLines={2} style={styles.title}>{idea.title}</Text><Text style={styles.meta}>{idea.status === 'processing' ? 'Processing now' : 'Ready'}</Text></View>{onToggleStar ? <Pressable accessibilityLabel={idea.starred ? `Unstar ${idea.title}` : `Star ${idea.title}`} accessibilityRole="button" hitSlop={8} onPress={(event) => { event.stopPropagation(); onToggleStar(idea.id); }} style={[styles.starButton, idea.starred && { backgroundColor: strongAccents[idea.accent] }]}><Star color={colors.ink} size={17} weight={idea.starred ? 'fill' : 'regular'} /></Pressable> : null}<View style={styles.open}><ArrowUpRight color={colors.inkMuted} size={17} weight="bold" /></View></Pressable>;
}
