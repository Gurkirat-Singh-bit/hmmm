...
export function OnboardingFooter({ step, complete, saving, onBack, onContinue }: {
  ...
  return (
    <View style={styles.footer}>
      {step > 0 ? <Pressable accessibilityLabel="Previous step" accessibilityRole="button" accessibilityState={{ disabled: saving }} disabled={saving} onPress={onBack} style={({ pressed }) => [styles.back, pressed && styles.pressed, saving && styles.disabled]}><ArrowLeft color={colors.inkInverse} size={20} weight="bold" /></Pressable> : null}
...
const styles = StyleSheet.create({
  ...
  back: { width: 56, minHeight: 60, alignItems: 'center', justifyContent: 'center', borderRadius: radii.medium, backgroundColor: colors.darkSurface },
