...
export function ReportUpdateProposal({ canApply, onApply, proposal }: {
  canApply: boolean;
    ...
    <View style={styles.card}>
      ...
      {decision === 'pending' ? (
        <View style={styles.actions}>
          <Pressable accessibilityHint="Writes this proposed report revision after confirmation" accessibilityRole="button" disabled={!canApply} onPress={() => void apply()} style={({ pressed }) => [styles.apply, !canApply && styles.disabled, pressed && styles.pressed]}><Text style={styles.applyText}>Update saved idea</Text></Pressable>
          <View style={styles.keep}><Text style={styles.keepText}>{canApply ? 'Nothing changes until you confirm.' : 'This proposal is based on an older report.'}</Text></View>
        </View>
      ) : (
        <View accessibilityLiveRegion="polite" style={[styles.decision, decision === 'applied' && styles.decisionApplied]}>
          <Text style={[styles.decisionText, decision === 'applied' && styles.decisionTextApplied]}>{decision === 'applying' ? 'Updating saved idea…' : decision === 'applied' ? 'Saved idea updated' : 'This proposal needs the latest report.'}</Text>
        </View>
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  reason: { marginTop: 7, color: colors.inkMuted, fontFamily: onboardingFonts.bodyMedium, fontSize: 12, lineHeight: 17 },
  ...
  keepText: { color: colors.inkMuted, fontFamily: onboardingFonts.bodyMedium, fontSize: 11, lineHeight: 15 },
