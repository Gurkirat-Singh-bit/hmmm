...
export function IdeaDiscussion({ applyProposal, composer, data, notice, onRetry, onSend, onSetComposer, sending }: {
  ...
  useEffect(() => {
    const isStreamingUpdate = data.messages.length === previousMessageCountRef.current
  ...
  }, [data.messages.length, lastMessage?.content, lastMessage?.id, lastMessage?.status]);
  ...
  return <SafeAreaView edges={['top', 'bottom']} style={styles.safeArea}>
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.keyboard}>
      <View style={styles.header}>
        <Pressable accessibilityLabel="Back to Discuss" accessibilityRole="button" onPress={() => router.canGoBack() ? router.back() : router.replace('/discuss')} style={({ pressed }) => [styles.circle, pressed && styles.pressed]}><ArrowLeft color={colors.ink} size={21} weight="bold" /></Pressable>
        <View style={styles.headerCopy}><Text accessibilityLabel={title} accessibilityRole="header" style={styles.title}>{title}</Text><Text style={styles.saved}>Saved locally</Text></View>
        <Pressable accessibilityLabel="View idea report" accessibilityRole="button" onPress={() => router.push(`/vault/${data.capture.id}`)} style={({ pressed }) => [styles.reportButton, pressed && styles.pressed]}><Text style={styles.reportText}>View idea</Text></Pressable>
      </View>
      ...
      <ScrollView
        ...
        onContentSizeChange={(_, height) => {
          ...
          if (height <= viewportHeightRef.current + SCROLL_BOTTOM_THRESHOLD) nearBottomRef.current = true;
          else if (previousContentHeight === 0) nearBottomRef.current = false;
        }}
        onLayout={(event) => {
          viewportHeightRef.current = event.nativeEvent.layout.height;
          if (contentHeightRef.current > 0 && contentHeightRef.current <= viewportHeightRef.current + SCROLL_BOTTOM_THRESHOLD) nearBottomRef.current = true;
        }}
        onScroll={(event) => {
          const { contentOffset, contentSize, layoutMeasurement } = event.nativeEvent;
          nearBottomRef.current = contentSize.height - (contentOffset.y + layoutMeasurement.height) <= SCROLL_BOTTOM_THRESHOLD;
        }}
        ref={scrollRef}
        scrollEventThrottle={16}
        showsVerticalScrollIndicator={false}
      >
        {data.messages.length ? data.messages.map((message) => <View key={message.id} style={styles.messageGroup}>
          <DiscussionMessage message={message} onRetry={message.role === 'assistant' ? (mode) => onRetry(message.id, mode) : undefined} />
          {message.role === 'assistant' && message.reportUpdateProposal ? <ReportUpdateProposal canApply={data.report?.revision === message.reportUpdateProposal.baseRevision} onApply={applyProposal} proposal={message.reportUpdateProposal} /> : null}
        </View>) : <ConversationEmpty />}
      </ScrollView>

      {!data.messages.length ? <DiscussionPromptChips disabled={sending} onChoose={onSetComposer} /> : null}
      <DiscussionComposer availability={data.availability} onChange={onSetComposer} onSend={onSend} sending={sending} value={composer} />
    </KeyboardAvoidingView>
  </SafeAreaView>;
}

function ConversationEmpty() {
  return <View style={styles.empty}><Text style={styles.emptyTitle}>Start with what feels uncertain.</Text><Text style={styles.emptyBody}>Ask a question or choose a direction below. Your messages stay on this device.</Text></View>;
}
...
const styles = StyleSheet.create({
  ...
  reportButton: { minHeight: 48, justifyContent: 'center', paddingHorizontal: 14, borderRadius: radii.pill, backgroundColor: colors.surfaceMuted },
  reportText: { color: colors.ink, fontFamily: onboardingFonts.bodyBold, fontSize: 11 },
  notice: { marginHorizontal: 20, marginTop: 10, color: colors.inkSecondary, fontFamily: onboardingFonts.bodyMedium, fontSize: 12, lineHeight: 17 },
