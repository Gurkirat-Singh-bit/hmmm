...
export function SettingsSubpage({ children, supporting, title }: { children: ReactNode; supporting: string; title: string }) {
  ...
  return <SafeAreaView edges={['top', 'bottom']} style={styles.safeArea}><View style={styles.header}><Pressable accessibilityLabel="Go back to settings" accessibilityRole="button" hitSlop={6} onPress={goBack} style={({ pressed }) => [styles.back, pressed && styles.pressed]}><ArrowLeft color={colors.ink} size={22} weight="bold" /></Pressable></View><KeyboardAvoidingView behavior="height" style={styles.keyboard}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}><Text accessibilityRole="header" style={styles.title}>{title}</Text><Text style={styles.supporting}>{supporting}</Text><View style={styles.body}>{children}</View></ScrollView></KeyboardAvoidingView></SafeAreaView>;
}

const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: colors.canvasSoft }, header: { minHeight: 64, justifyContent: 'center', paddingHorizontal: spacing.page }, keyboard: { flex: 1 },
  back: { width: 48, height: 48, alignItems: 'center', justifyContent: 'center', borderRadius: radii.pill, backgroundColor: colors.canvas },
