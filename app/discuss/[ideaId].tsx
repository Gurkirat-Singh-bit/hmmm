...
export default function IdeaDiscussionRoute() {
  ...
  if (thread.loading) return <DiscussionNotFound kind="loading" onBack={() => router.replace('/discuss')} onVault={() => router.replace('/vault')} />;
  if (thread.error) return <DiscussionNotFound kind="error" onBack={() => router.replace('/discuss')} onRetry={thread.refresh} onVault={() => router.replace('/vault')} />;
  if (!thread.data) return <DiscussionNotFound onBack={() => router.replace('/discuss')} onVault={() => router.replace('/vault')} />;
